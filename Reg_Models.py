...
def obj_extra_trees(trial):
    e_n_estimators = trial.suggest_int("n_estimators", low=20, high=200, step=10)
    e_min_samples_split = trial.suggest_int("min_samples_split", low=2, high=8, step=1)
    e_min_samples_leaf = trial.suggest_int("min_samples_leaf", low=1, high=8, step=1)
...
def obj_ada_boost(trial):
    h_n_estimators = trial.suggest_int("n_estimators", low=20, high=500, step=10)
...
def obj_catBoostRegressor(trial):
    cat_n_estimators = trial.suggest_int("n_estimators", low=20, high=500, step=10)
...
def obj_LightBoost(trial):
    ...
    light_num_leaves = trial.suggest_int("num_leaves", low=50,high=300,step=5)
    light_estimators = trial.suggest_int("num_estimators", low=100,high=800,step=10)
